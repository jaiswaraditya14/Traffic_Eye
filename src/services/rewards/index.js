...
export const rewardService = {
    ...
    async getUserReportHistory(limit = 10) {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] Report history unavailable.');
    ...
    async awardBaseSubmissionPoints(reportId) {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] Point award failed.');
    ...
    async redeemItem(item) {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] Redemption failed.');
            return { success: false, error: error.message };
        }
    },
    ...
    async getRedeemedItems() {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] Redeemed items unavailable.');
    ...
    async getActivityHistory(limit = 40) {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] Activity history unavailable.');
            return { success: false, history: [], error: error.message };
        }
    },
    ...
    async clearUserHistory() {
        try {
            ...
            if (__DEV__) console.warn('[RewardService] History cleanup failed.');
            return { success: false, error: error.message };
        }
    }
};
